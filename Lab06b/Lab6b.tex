...
\section{Procedimentos e coleta de dados}
	...
	\section{Análise dos resultados}
		...
		\subsection{Calor latente de fusão do gelo}
			Sabendo que 
			\begin{equation}
				Q_{calorimetro} + Q_{gelo} + Q_{fusão} + Q_{gelo-agua} + Q_{agua} = 0
			\end{equation}
			ou seja:
			
			$$ C_{c}\Delta T + m_{gelo}c_{gelo}\Delta T + m_{gelo}L_{f} + m_{gelo}c_{agua}\Delta T + m_{agua}c_{agua}\Delta T = 0 \leftrightarrow L_{f} = -(T_{f}-T_{a})(C_{c}+m_{a}c_{a}) - m_{g}c_{a}$$
 IMCOMPLETO !!! TERMINO MAIS TARDE
