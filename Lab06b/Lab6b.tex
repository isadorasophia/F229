...
\makeatletter
\providecommand{\left(}{(} %define
\providecommand{\right)}{)} %define
...
\begin{document}
	...
	\section{Análise dos resultados}
		...
		\subsection{Cálculo do calor específico dos metais}
			...
			\subsubsection{Encontrando o calor específico de cada metal}
				...
				\begin{equation}
					...
					\left(\sigma_{m_{a}}c_{a}\right)^{2}}}{m_{M}^{2}}+c_{m}\cdot
					\sqrt{\cfrac{\sigma_{m_{M}}^{2}}{m_{M}^{2}}+\cfrac{\sigma_{\Delta T}^{2}}{
		...
		\subsection{Calor latente de fusão do gelo}
			...
			...ou seja:
			\hspace{-2cm}%
			$$ C\Delta T + m_{\acute{A}gua}c_{\acute{A}gua}\Delta T + m_{Gelo}c_{Gelo}\Delta T +  m_{Gelo}L_{f} + m_{Gelo}c_{\acute{A}gua}\Delta T = 0 \leftrightarrow$$
			
			

			\begin{equation}
			 	L_{f}=\cfrac{\left(T_{a}-T_{e}\right)\left(C+m_{a} \cdot c_{a}\right)}{m_{g}} + c_{g} \cdot T_{g} - c_{a} \cdot T_{e}
			\end{equation}
		...
		$
			...
			$ =
				\left(\sigma_{T_{g}}\cdot c_{g}\right)^{2}+\left(\sigma_{T_{e}}\cdot\cfrac{C+c_{a}\cdot
				\left(m_{a}+m_{g}\right)}{m_{g}}\right)^{2}+\left(\sigma_{T_{a}}\cdot\cfrac{C+m_{a}c_{a}}{m_{g}}\right)^{2}+
				\left(\sigma_{m_{a}}\cdot c_{a}\cdot\cfrac{T_{a}-T_{e}}{m_{g}}\right)^{2}+
				\left(L_{f}\cdot\cfrac{\sigma_{m_{g}}}{m_{g}}\right)^{2}+
			...
			Como o valor do calor latente da água é de 79.53 $\frac{cal}{g}$, nosso experimento não obteve êxito.
